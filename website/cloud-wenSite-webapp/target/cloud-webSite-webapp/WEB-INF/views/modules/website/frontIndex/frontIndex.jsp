...
<div class="slider" id="slider">
    <div class="slider-inner">
        <c:forEach var="banner" items="${banner}">
            <div class="item">
                <a href="${frontPath}${banner.url}"><img class="img" src="${ctxImg}${banner.picPath}"></a>
            </div>
        </c:forEach>
    </div>
</div>
...
<div class="context">
    ...
    <div class="context_left" style="background-image: url(${ctxImg}${hotNews.picPath});background-repeat:no-repeat;position: relative; width: auto; height: 394px;">
        <div class="styleText" style=" margin:0px;margin-top:343px;text-align: center;background: rgba(0,0,0, 0.4);height: 50px; width:554px;"><a href="${frontPath}/wapNews?id=${hotNews.id}"
...
<%--news list ,search three new for here --%>
        <div class="context_right">
            ...
            <div >
                <img  onclick="window.location.href='${frontPath}/wapNews?id=${news.id}'" src="${ctxImg}${news.picPath}"/>
                <div class="content_text " >
                    <div onclick="window.location.href='${frontPath}/wapNews?id=${news.id}'" class="styleText">${news.title}</div>
                    <div class="styleTime"><fmt:formatDate value="${news.addTime}" pattern="yyyy年-MM月-dd日 HH:mm:ss"/></div>
                    <div class="border_ss"><a href="${frontPath}/wapNews?id=${news.id}" class="colose">More></a></div>
                </div>
            </div>
            </c:forEach>
        </div>
